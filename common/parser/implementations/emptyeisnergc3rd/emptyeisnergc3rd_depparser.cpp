...
namespace emptyeisnergc3rd {
    ...
    void DepParser::train(const DependencyTree &correct, const int &round) {
        ...
        if (m_nState == ParserState::GOLDTEST) {
            ...
            for (const auto &quararc : m_vecCorrectQuarArcs) {
                ...
                m_setTriSiblingArcGoldScore.insert(
                        QuarGram<int>(quararc.second(), quararc.third(), quararc.forth(), quararc.fifth()));
                if (quararc.first() != -1) {
                    m_setGrandSiblingArcGoldScore.insert(
                            TriGram<int>(quararc.first(), quararc.second(), quararc.fifth()));
                    m_setGrandBiSiblingArcGoldScore.insert(
                            QuarGram<int>(quararc.first(), quararc.second(), quararc.forth(), quararc.fifth()));
                    m_setGrandTriSiblingArcGoldScore.insert(
                            QuinGram<int>(quararc.first(), quararc.second(), quararc.third(), quararc.forth(),
                                          quararc.fifth()));
                }
            }
        }
    ...
    void DepParser::decode() {
            ...
            for (int i = 0, max_i = m_nSentenceLength - d + 1; i < max_i; ++i) {
                ...
                for (int s = i; s < l; ++s) {
                    ...
                    do {
                        ...
                        // l2r_empty_inside
                        tscore l_empty_inside_base_score =
                                l2r_arc_score + ritem.r2l[GRAND_INDEX(i, s + 1, l - s)].getScore();
                        for (const auto &l_beam : litem.l2r_empty_outside[lgrand]) {
                            ...
                            item.updateL2REmptyInside(grand, p, l_beam.getSplit(), l_empty_inside_base_score +
                                                                                   ...
                                                                                   triSiblingArcScore(i,
                                                                                                      j == i ? -1 : j,
                                                                                                      k, l) +
                                                                                   m_lGrandSiblingArcScore[ENCODE_L2R(
                                                                                           g)] +
                                                                                   grandBiSiblingArcScore(g, i, k, l) +
                                                                                   grandTriSiblingArcScore(g, i,
                                                                                                           j == i ? -1
                                                                                                                  : j,
                                                                                                           k, l));
                        }
                        ...
                        // r2l_empty_inside
                        tscore r_empty_inside_base_score =
                                r2l_arc_score + litem.l2r[GRAND_INDEX(l, i, s - i + 1)].getScore();
                        for (const auto &r_beam : ritem.r2l_empty_outside[rgrand]) {
                            ...
                            item.updateR2LEmptyInside(grand, p, r_beam.getSplit(), r_empty_inside_base_score +
                                                                                   ...
                                                                                   triSiblingArcScore(l,
                                                                                                      j == l ? -1 : j,
                                                                                                      k, i) +
                                                                                   m_lGrandSiblingArcScore[ENCODE_R2L(
                                                                                           g)] +
                                                                                   grandBiSiblingArcScore(g, l, k, i) +
                                                                                   grandTriSiblingArcScore(g, l,
                                                                                                           j == l ? -1
                                                                                                                  : j,
                                                                                                           k, i));
                        }
                ...
                for (int k = i + 1; k < l; ++k) {
                    ...
                    tscore l_base_jux =
                            ritem.jux[rgrand].getScore() + l2r_arc_score + m_lBiSiblingArcScore[ENCODE_2ND_L2R(i, k)];
                    tscore r_base_jux =
                            litem.jux[lgrand].getScore() + r2l_arc_score + m_lBiSiblingArcScore[ENCODE_2ND_R2L(i, k)];
                    ...
                    do {
                        ...
                        // l2r solid both
                        tscore l_base_score = l_base_jux + m_lGrandSiblingArcScore[ENCODE_L2R(g)] +
                                              m_lGrandBiSiblingArcScore[ENCODE_L2R(g)];

                        for (const auto &sws : l_beam) {
                            const int &j = sws.getSplit();
                            item.updateL2RSolidBoth(grand, k, j, l_base_score +
                                                                 sws.getScore() +
                                                                 triSiblingArcScore(i, j == i ? -1 : IS_EMPTY(j) ? j + 1
                                                                                                                 : j, k,
                                                                                    l) +
                                                                 grandTriSiblingArcScore(g, i,
                                                                                         j == i ? -1 : IS_EMPTY(j) ? j +
                        ...
                        // r2l solid both
                        tscore r_base_score = r_base_jux + m_lGrandSiblingArcScore[ENCODE_R2L(g)] +
                                              m_lGrandBiSiblingArcScore[ENCODE_R2L(g)];

                        for (const auto &sws : r_beam) {
                            const int &j = sws.getSplit();
                            item.updateR2LSolidBoth(grand, k, j, r_base_score +
                                                                 sws.getScore() +
                                                                 triSiblingArcScore(l, j == l ? -1 : IS_EMPTY(j) ? j + 1
                                                                                                                 : j, k,
                                                                                    i) +
                                                                 grandTriSiblingArcScore(g, l,
                                                                                         j == l ? -1 : IS_EMPTY(j) ? j +
                        ...
                        for (const auto &swt : m_abFirstOrderEmptyScore[ENCODE_L2R(i)]) {
                            ...
                            tscore l_empty_outside_base_score =
                                    ritem.l2r[rgrand].getScore() + swt.getScore() + biSiblingArcScore(i, k, o);
                        ...
                        for (const auto &swt : m_abFirstOrderEmptyScore[ENCODE_R2L(i)]) {
                            ...
                            tscore r_empty_outside_base_score =
                                    litem.r2l[lgrand].getScore() + swt.getScore() + biSiblingArcScore(l, k, o);
                        ...
                        // complete
                        item.updateL2R(grand, k,
                                       litem.l2r_solid_outside[GRAND_INDEX(g, i, k - i + 1)].bestItem().getScore() +
                                       ritem.l2r[rgrand].getScore());
                        item.updateR2L(grand, k,
                                       ritem.r2l_solid_outside[GRAND_INDEX(g, k, l - k + 1)].bestItem().getScore() +
                                       litem.r2l[lgrand].getScore());
                ...
                if (d > 1) {
                    ...
                    do {
                        ...
                        // l2r_solid_both
                        item.updateL2RSolidBoth(grand, i, i,
                                                m_lItems[d - 1][i + 1].r2l[GRAND_INDEX(i, i + 1, d - 1)].getScore() +
                                                l2r_arc_score +
                        ...
                        // r2l_solid_both
                        item.updateR2LSolidBoth(grand, l, l,
                                                m_lItems[d - 1][i].l2r[GRAND_INDEX(l, i, d - 1)].getScore() +
                                                r2l_arc_score +
                ...
                do {
                    ...
                    for (const auto &swt : m_abFirstOrderEmptyScore[ENCODE_L2R(i)]) {
                        ...
                        int o = ENCODE_EMPTY(l + 1, t);
                        if (d > 1) {
                            tscore base_l2r_empty_outside_score = swt.getScore() + biSiblingArcScore(i, l, o) +
                                                                  m_lItems[1][l].l2r[GRAND_INDEX(i, l, 1)].getScore();
                    ...
                    for (const auto &swt : m_abFirstOrderEmptyScore[ENCODE_R2L(i)]) {
                        ...
                        int o = ENCODE_EMPTY(i, t);
                        if (d > 1) {
                            tscore base_r2l_empty_outside_score = swt.getScore() + biSiblingArcScore(l, i, o) +
                                                                  m_lItems[1][i].r2l[GRAND_INDEX(l, i, 1)].getScore();
                    ...
                    // l2r
                    if (d > 1) {
                        item.updateL2R(grand, l, item.l2r_solid_outside[grand].bestItem().getScore() +
                                                 m_lItems[1][l].l2r[GRAND_INDEX(i, l, 1)].getScore());
                    }
                    else {
                    ...
                    if (item.l2r_empty_outside[grand].size() > 0) {
                        item.updateL2R(grand, item.l2r_empty_outside[grand].bestItem().getSplit(),
                                       item.l2r_empty_outside[grand].bestItem().getScore());
                    }
                    // r2l
                    if (d > 1) {
                        item.updateR2L(grand, i, item.r2l_solid_outside[grand].bestItem().getScore() +
                                                 m_lItems[1][i].r2l[GRAND_INDEX(l, i, 1)].getScore());
                    }
                    else {
                    ...
                    if (item.r2l_empty_outside[grand].size() > 0) {
                        item.updateR2L(grand, item.r2l_empty_outside[grand].bestItem().getSplit(),
                                       item.r2l_empty_outside[grand].bestItem().getScore());
                    }
        ...
        for (int m = 0; m < m_nSentenceLength; ++m) {
            item.updateR2L(m_nSentenceLength, m,
                           m_lItems[m + 1][0].r2l[GRAND_INDEX(m_nSentenceLength, 0, m + 1)].getScore() +
                           m_lItems[m_nSentenceLength - m][m].l2r[GRAND_INDEX(m_nSentenceLength, m,
                                                                              m_nSentenceLength - m)].getScore() +
        ...
        while (!stack.empty()) {
            ...
            switch (item.type) {
                ...
                case StateItem::L2R:
                    ...
                    stack.push(std::tuple<int, int, int, int>(grand, split - item.left + 1, m_lItems[split - item.left +
                                                                                                     1][item.left].l2r_solid_outside[GRAND_INDEX(
                            grand, item.left, split - item.left + 1)].bestItem().getSplit(), item.left));
                ...
                case StateItem::R2L:
                    ...
                    stack.push(std::tuple<int, int, int, int>(grand, item.right - split + 1,
                                                              m_lItems[item.right - split +
                                                                       1][split].r2l_solid_outside[GRAND_INDEX(grand,
                                                                                                               split,
                                                                                                               item.right -
                                                                                                               split +
                                                                                                               1)].bestItem().getSplit(),
                ...
                case StateItem::L2R_EMPTY_OUTSIDE:
                    m_vecTrainArcs.push_back(
                            BiGram<int>(item.left, ENCODE_EMPTY(item.right + 1, DECODE_EMPTY_TAG(split))));
    ...
    void DepParser::generate(DependencyTree *retval, const DependencyTree &correct) {
        ...
        std::sort(emptyArcs.begin(), emptyArcs.end(),
                  [](const Arc &arc1, const Arc &arc2) { return compareArc(arc1, arc2); });
        ...
        while (itr_e != emptyArcs.end() && idx != m_nSentenceLength) {
            if (idx < DECODE_EMPTY_POS(itr_e->second())) {
                ...
                retval->push_back(DependencyTreeNode(
                        POSTaggedWord(TEmptyTag::key(DECODE_EMPTY_TAG((itr_e++)->second())), "EMCAT"), -1, "NMOD"));
            }
        }
        ...
        while (itr_e != emptyArcs.end()) {
            nidmap[itr_e->second() * 256 + itr_e->first()] = nidx++;
            retval->push_back(
                    DependencyTreeNode(POSTaggedWord(TEmptyTag::key(DECODE_EMPTY_TAG((itr_e++)->second())), "EMCAT"),
                                       -1, "NMOD"));
        }

        for (const auto &arc : m_vecTrainArcs) {
            TREENODE_HEAD(retval->at(nidmap[IS_EMPTY(arc.second()) ? arc.second() * 256 + arc.first()
                                                                   : arc.second()])) = nidmap[arc.first()];
        }
    }
    ...
    const tscore &DepParser::arcScore(const int &p, const int &c) {
        if (m_nState == ParserState::GOLDTEST) {
            m_nRetval = m_setArcGoldScore.find(BiGram<int>(p, c)) == m_setArcGoldScore.end() ? GOLD_NEG_SCORE
                                                                                             : GOLD_POS_SCORE;
            return m_nRetval;
        }
        m_nRetval = 0;
    ...
    const tscore &DepParser::biSiblingArcScore(const int &p, const int &c, const int &c2) {
        if (m_nState == ParserState::GOLDTEST) {
            m_nRetval = m_setBiSiblingArcGoldScore.find(TriGram<int>(p, c, c2)) == m_setBiSiblingArcGoldScore.end()
                        ? GOLD_NEG_SCORE : GOLD_POS_SCORE;
            return m_nRetval;
        }
        m_nRetval = 0;
    ...
    const tscore &DepParser::triSiblingArcScore(const int &p, const int &c, const int &c2, const int &c3) {
        if (m_nState == ParserState::GOLDTEST) {
            m_nRetval =
                    m_setTriSiblingArcGoldScore.find(QuarGram<int>(p, c, c2, c3)) == m_setTriSiblingArcGoldScore.end()
                    ? GOLD_NEG_SCORE : GOLD_POS_SCORE;
            return m_nRetval;
        }
        m_nRetval = 0;
    ...
    const tscore &DepParser::grandSiblingArcScore(const int &g, const int &p, const int &c) {
        if (m_nState == ParserState::GOLDTEST) {
            m_nRetval = m_setGrandSiblingArcGoldScore.find(TriGram<int>(g, p, c)) == m_setGrandSiblingArcGoldScore.end()
                        ? GOLD_NEG_SCORE : GOLD_POS_SCORE;
            return m_nRetval;
        }
        m_nRetval = 0;
    ...
    const tscore &DepParser::grandBiSiblingArcScore(const int &g, const int &p, const int &c, const int &c2) {
        if (m_nState == ParserState::GOLDTEST) {
            m_nRetval = m_setGrandBiSiblingArcGoldScore.find(QuarGram<int>(g, p, c, c2)) ==
                        m_setGrandBiSiblingArcGoldScore.end() ? GOLD_NEG_SCORE : GOLD_POS_SCORE;
            return m_nRetval;
        }
        m_nRetval = 0;
    ...
    const tscore &DepParser::grandTriSiblingArcScore(const int &g, const int &p, const int &c, const int &c2,
                                                     const int &c3) {
        if (m_nState == ParserState::GOLDTEST) {
            m_nRetval = m_setGrandTriSiblingArcGoldScore.find(QuinGram<int>(g, p, c, c2, c3)) ==
                        m_setGrandTriSiblingArcGoldScore.end() ? GOLD_NEG_SCORE : GOLD_POS_SCORE;
            return m_nRetval;
        }
        m_nRetval = 0;
    ...
    void DepParser::readEmptySentAndArcs(const DependencyTree &correct) {
        ...
        for (const auto &node : tcorrect) {
            if (TREENODE_POSTAG(node) == EMPTYTAG) {
                ...
                m_lSentence[m_nSentenceLength][0].refer(TWord::code(TREENODE_WORD(node)),
                                                        TPOSTag::code(TREENODE_POSTAG(node)));
                for (int i = TEmptyTag::start(), max_i = TEmptyTag::count(); i < max_i; ++i) {
                    m_lSentence[m_nSentenceLength][i].refer(
                            TWord::code(
                                    (m_nSentenceLength == 0 ? START_WORD : TWord::key(
                                            m_lSentence[m_nSentenceLength - 1][0].first())) +
        ...
        for (const auto &node : tcorrect) {
            if (TREENODE_POSTAG(node) == EMPTYTAG) {
                ...
                m_vecCorrectArcs.push_back(
                        Arc(TREENODE_HEAD(node), ENCODE_EMPTY(idx, TEmptyTag::code(TREENODE_WORD(node)))));
            }
            else {
                m_lSentenceWithEmpty[idxwe++].refer(TWord::code(TREENODE_WORD(node)),
                                                    TPOSTag::code(TREENODE_POSTAG(node)));
    ...
    void DepParser::getOrUpdateSiblingScore(const int &p, const int &c, const int &c2, const int &c3,
                                            const int &amount) {
        Weight *cweight = (Weight *) m_Weight;
        ...
        cweight->m_mapPwC1pC2pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        ...
        cweight->m_mapC1wPpC2pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        word_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapC1wPpC2pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        ...
        cweight->m_mapC2wPpC1pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        word_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapC2wPpC1pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        ...
        cweight->m_mapC3wPpC1pC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        word_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapC3wPpC1pC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        ...
        cweight->m_mapPwC1wC2pC3p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        word_word_tag_tag_int.referLast(m_nDir);
        cweight->m_mapPwC1wC2pC3p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        ...
        cweight->m_mapPwC2wC1pC3p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        word_word_tag_tag_int.referLast(m_nDir);
        cweight->m_mapPwC2wC1pC3p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        ...
        cweight->m_mapPwC3wC1pC2p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        word_word_tag_tag_int.referLast(m_nDir);
        cweight->m_mapPwC3wC1pC2p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        ...
        cweight->m_mapC1wC2wPpC3p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        word_word_tag_tag_int.referLast(m_nDir);
        cweight->m_mapC1wC2wPpC3p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        ...
        cweight->m_mapC1wC3wPpC2p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        word_word_tag_tag_int.referLast(m_nDir);
        cweight->m_mapC1wC3wPpC2p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        ...
        cweight->m_mapC2wC3wPpC1p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        word_word_tag_tag_int.referLast(m_nDir);
        cweight->m_mapC2wC3wPpC1p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        ...
        cweight->m_mapPpC1pC2pC3p.getOrUpdateScore(m_nRetval, tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
        tag_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapPpC1pC2pC3p.getOrUpdateScore(m_nRetval, tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                   m_nTrainingRound);
    ...
    void DepParser::getOrUpdateGrandScore(const int &g, const int &p, const int &c, const int &c2, const int &amount) {
        ...
        cweight->m_mapGwPpCpC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        word_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapGwPpCpC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        ...
        cweight->m_mapPwGpCpC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        word_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapPwGpCpC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        ...
        cweight->m_mapCwGpPpC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        word_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapCwGpPpC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        ...
        cweight->m_mapC2wGpPpCp.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        word_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapC2wGpPpCp.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        ...
        cweight->m_mapGwPwCpC2p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        word_word_tag_tag_int.referLast(m_nDir);
        cweight->m_mapGwPwCpC2p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        ...
        cweight->m_mapGwCwPpC2p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        word_word_tag_tag_int.referLast(m_nDir);
        cweight->m_mapGwCwPpC2p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        ...
        cweight->m_mapGwC2wPpCp.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        word_word_tag_tag_int.referLast(m_nDir);
        cweight->m_mapGwC2wPpCp.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        ...
        cweight->m_mapPwCwGpC2p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        word_word_tag_tag_int.referLast(m_nDir);
        cweight->m_mapPwCwGpC2p.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        ...
        cweight->m_mapCwC2wGpPp.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        word_word_tag_tag_int.referLast(m_nDir);
        cweight->m_mapCwC2wGpPp.getOrUpdateScore(m_nRetval, word_word_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        ...
        cweight->m_mapGpPpCpC2p.getOrUpdateScore(m_nRetval, tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
        tag_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapGpPpCpC2p.getOrUpdateScore(m_nRetval, tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                 m_nTrainingRound);
    ...
    void DepParser::getOrUpdateGrandScore(const int &g, const int &p, const int &c, const int &c2, const int &c3,
                                          const int &amount) {
        Weight *cweight = (Weight *) m_Weight;
        ...
        cweight->m_mapGwPpCpC2pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                    m_nTrainingRound);
        ...
        cweight->m_mapPwGpCpC2pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                    m_nTrainingRound);
        word_tag_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapPwGpCpC2pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                    m_nTrainingRound);
        ...
        cweight->m_mapCwGpPpC2pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                    m_nTrainingRound);
        word_tag_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapCwGpPpC2pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                    m_nTrainingRound);
        ...
        cweight->m_mapC2wGpPpCpC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                    m_nTrainingRound);
        word_tag_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapC2wGpPpCpC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                    m_nTrainingRound);
        ...
        cweight->m_mapC3wGpPpCpC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                    m_nTrainingRound);
        word_tag_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapC3wGpPpCpC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                    m_nTrainingRound);
        ...
        cweight->m_mapGpPpCpC2pC3p.getOrUpdateScore(m_nRetval, tag_tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                    m_nTrainingRound);
        ...
        cweight->m_mapGwCpC2pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                  m_nTrainingRound);
        word_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapGwCpC2pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                  m_nTrainingRound);
        ...
        cweight->m_mapCwGpC2pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                  m_nTrainingRound);
        word_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapCwGpC2pC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                  m_nTrainingRound);
        ...
        cweight->m_mapC2wGpCpC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                  m_nTrainingRound);
        word_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapC2wGpCpC3p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                  m_nTrainingRound);
        ...
        cweight->m_mapC3wGpCpC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                  m_nTrainingRound);
        word_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapC3wGpCpC2p.getOrUpdateScore(m_nRetval, word_tag_tag_tag_int, m_nScoreIndex, amount,
                                                  m_nTrainingRound);
        ...
        cweight->m_mapGpCpC2pC3p.getOrUpdateScore(m_nRetval, tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                  m_nTrainingRound);
        tag_tag_tag_tag_int.referLast(m_nDir);
        cweight->m_mapGpCpC2pC3p.getOrUpdateScore(m_nRetval, tag_tag_tag_tag_int, m_nScoreIndex, amount,
                                                  m_nTrainingRound);
