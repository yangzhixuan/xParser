...
namespace emptyeisner {
    ...
    void DepParser::train(const DependencyTree &correct, const int &round) {
        ...
        if (m_nState == ParserState::GOLDTEST) {
            ...
            for (const auto &triarc : m_vecCorrectTriArcs) {
                ...
                m_setThirdGoldScore.insert(
                        QuarGram<int>(triarc.first(), triarc.second(), triarc.third(), triarc.forth()));
            }
        }
        ...
        for (int d = 1; d <= m_nSentenceLength + 1; ++d) {
            ...
            for (int i = 0, max_i = m_nSentenceLength - d + 1; i < max_i; ++i) {
                ...
                for (int k = i + 1; k < l; ++k) {
                    ...
                    // l2r_solid_both
                    tscore l_solid_both_base_score =
                            ritem.jux.getScore() + l2r_arc_score + m_lSecondOrderScore[ENCODE_2ND_L2R(i, k)];
                    ...
                    for (const auto &swt : m_abFirstOrderEmptyScore[ENCODE_L2R(i)]) {
                        ...
                        tscore l_empty_outside_base_score =
                                ritem.l2r.getScore() + swt.getScore() + twoArcScore(i, k, o);
                        for (const auto &swbs : l_beam) {
                            // j is inner split
                            const int &j = swbs.getSplit();
                            item.updateL2REmptyOutside(s, j, l_empty_outside_base_score +
                                                             swbs.getScore() +
                                                             triArcScore(i, j == i ? -1 : IS_EMPTY(j) ? j + 1 : j, k,
                                                                         o));
                        }
                    }
                    // l2r
                    ...
                    // r2l_solid_both
                    tscore r_solid_both_base_score =
                            litem.jux.getScore() + r2l_arc_score + m_lSecondOrderScore[ENCODE_2ND_R2L(i, k)];
                    ...
                    for (const auto &swt : m_abFirstOrderEmptyScore[ENCODE_R2L(i)]) {
                        ...
                        tscore r_empty_outside_base_score =
                                litem.r2l.getScore() + swt.getScore() + twoArcScore(l, k, o);
                        for (const auto &swbs : r_beam) {
                            const int &j = swbs.getSplit();
                            item.updateR2LEmptyOutside(s, j, r_empty_outside_base_score +
                                                             swbs.getScore() +
                                                             triArcScore(l, j == l ? -1 : IS_EMPTY(j) ? j + 1 : j, k,
                                                                         o));
                        }
                    }
                    // r2l
                ...
                if (item.l2r_empty_outside.size() > 0) {
                    item.updateL2R(item.l2r_empty_outside.bestItem().getSplit(),
                                   item.l2r_empty_outside.bestItem().getScore());
                }
                // r2l
                item.updateR2L(i, item.r2l_solid_outside.bestItem().getScore() + m_lItems[1][i].r2l.getScore());
                if (item.r2l_empty_outside.size() > 0) {
                    item.updateR2L(item.r2l_empty_outside.bestItem().getSplit(),
                                   item.r2l_empty_outside.bestItem().getScore());
                }
            ...
            if (d > 1) {
                ...
                // r2l_solid_outside
                item.updateR2LSolidOutside(m_nSentenceLength, m_nSentenceLength,
                                           m_lItems[d - 1][item.left].l2r.getScore() +
                ...
                // r2l
                item.updateR2L(item.left,
                               item.r2l_solid_outside.bestItem().getScore() + m_lItems[1][item.left].r2l.getScore());
                for (int i = item.left, s = item.left + 1, j = m_nSentenceLength + 1; s < j - 1; ++s) {
                    item.updateR2L(s, m_lItems[j - s][s].r2l_solid_outside.bestItem().getScore() +
                                      m_lItems[s - i + 1][i].r2l.getScore());
                }
        ...
        while (!stack.empty()) {
            ...
            switch (item.type) {
                ...
                case StateItem::L2R:
                    ...
                    stack.push(std::tuple<int, int, int>(split - item.left + 1, m_lItems[split - item.left +
                                                                                         1][item.left].l2r_solid_outside.bestItem().getSplit(),
                                                         item.left));
                ...
                case StateItem::R2L:
                    ...
                    stack.push(std::tuple<int, int, int>(item.right - split + 1, m_lItems[item.right - split +
                                                                                          1][split].r2l_solid_outside.bestItem().getSplit(),
                                                         split));
    ...
    void DepParser::goldCheck() {
        ...
        if (m_vecCorrectArcs.size() != m_vecTrainArcs.size() ||
            m_lItems[m_nSentenceLength + 1][0].r2l.getScore() / GOLD_POS_SCORE != 3 * m_vecTrainArcs.size()) {
            ++m_nTotalErrors;
            std::cout << "gold parse len error at " << m_nTrainingRound << std::endl;
            for (const auto &triarc : m_vecTrainTriArcs) {
                std::cout << triarc.first() << " " << triarc.second() << " " << triarc.third() << " " <<
                triarc.forth() << std::endl;
            }
            std::cout << std::endl;
        }
        else {
    ...
    const tscore &DepParser::arcScore(const int &p, const int &c) {
        if (m_nState == ParserState::GOLDTEST) {
            m_nRetval = m_setFirstGoldScore.find(BiGram<int>(p, c)) == m_setFirstGoldScore.end() ? GOLD_NEG_SCORE
                                                                                                 : GOLD_POS_SCORE;
            return m_nRetval;
        }
        m_nRetval = 0;
    ...
    const tscore &DepParser::twoArcScore(const int &p, const int &c, const int &c2) {
        if (m_nState == ParserState::GOLDTEST) {
            m_nRetval = m_setSecondGoldScore.find(TriGram<int>(p, c, c2)) == m_setSecondGoldScore.end() ? GOLD_NEG_SCORE
                                                                                                        : GOLD_POS_SCORE;
            return m_nRetval;
        }
        m_nRetval = 0;
    ...
    const tscore &DepParser::triArcScore(const int &p, const int &c, const int &c2, const int &c3) {
        if (m_nState == ParserState::GOLDTEST) {
            m_nRetval =
                    m_setThirdGoldScore.find(QuarGram<int>(p, c, c2, c3)) == m_setThirdGoldScore.end() ? GOLD_NEG_SCORE
                                                                                                       : GOLD_POS_SCORE;
            return m_nRetval;
        }
        m_nRetval = 0;
