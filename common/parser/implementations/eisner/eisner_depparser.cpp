...
namespace eisner {
    ...
    void DepParser::decode() {
        for (int d = 2; d <= m_nSentenceLength + 1; ++d) {
            ...
            item.updateR2LIm(m_nSentenceLength - 1,
                             m_lItems[d - 1][item.left].l2r.getScore() + m_lFirstOrderScore[ENCODE_R2L(item.left)]);
    ...
    void DepParser::goldCheck() {
        if (m_vecCorrectArcs.size() != m_vecTrainArcs.size() ||
            m_lItems[m_nSentenceLength + 1][0].r2l.getScore() / GOLD_POS_SCORE != m_vecTrainArcs.size()) {
