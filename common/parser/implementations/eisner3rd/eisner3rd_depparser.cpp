    ...
    void DepParser::decode() {
        ...
        for (int d = 2; d <= m_nSentenceLength + 1; ++d) {
            ...
            item.updateR2LSolidBoth(m_nSentenceLength, m_nSentenceLength, m_lItems[d - 1][item.left].l2r.getScore() +
                                                                          ...
                                                                          m_lSecondOrderScore[ENCODE_2ND_R2L(item.left,
                                                                                                             item.left)] +
            ...
            for (int i = item.left, s = item.left + 1, j = m_nSentenceLength + 1; s < j - 1; ++s) {
                item.updateR2L(s, m_lItems[j - s][s].r2l_solid_both.bestItem().getScore() +
            ...
            switch (item.type) {
                ...
                case StateItem::L2R:
                    ...
                    if (item.left < item.right - 1) {
                        ...
                        stack.push(std::tuple<int, int, int>(s - item.left + 1, m_lItems[s - item.left +
                                                                                         1][item.left].l2r_solid_both.bestItem().getSplit(),
                                                             item.left));
                ...
                case StateItem::R2L:
                    ...
                    if (item.left < item.right - 1) {
                        ...
                        stack.push(std::tuple<int, int, int>(item.right - s + 1, m_lItems[item.right - s +
                                                                                          1][s].r2l_solid_both.bestItem().getSplit(),
                                                             s));
                        ...
                        m_vecTrainArcs.push_back(
                                BiGram<int>(item.right == m_nSentenceLength ? -1 : item.right, item.left));
                    }
                    break;
