            ...
            for (int i = 0, max_i = m_nSentenceLength - d + 1; i < max_i; ++i) {
                ...
                for (int s = i; s < l; ++s) {
                    ...
                    do {
                        ...
                        item.updateJUX(GRAND_INDEX(g, i, d), s,
                                       m_lItems[s - i + 1][i].l2r[GRAND_INDEX(g, i, s - i + 1)].getScore() +
                                       m_lItems[l - s][s + 1].r2l[GRAND_INDEX(g, s + 1, l - s)].getScore());
                ...
                for (int k = i + 1; k < l; ++k) {
                    ...
                    do {
                        ...
                        item.updateL2R(grand, k,
                                       litem.l2r_solid_both[GRAND_INDEX(g, i, k - i + 1)].bestItem().getScore() +
                                       ritem.l2r[rgrand].getScore());
                        item.updateR2L(grand, k,
                                       ritem.r2l_solid_both[GRAND_INDEX(g, k, l - k + 1)].bestItem().getScore() +
            ...
            switch (item.type) {
                ...
                case StateItem::L2R:
                    ...
                    if (item.left < item.right - 1) {
                        ...
                        stack.push(std::tuple<int, int, int, int>(g, s - item.left + 1, m_lItems[s - item.left +
                                                                                                 1][item.left].l2r_solid_both[GRAND_INDEX(
                                g, item.left, s - item.left + 1)].bestItem().getSplit(), item.left));
                ...
                case StateItem::R2L:
                    ...
                    if (item.left < item.right - 1) {
                        ...
                        stack.push(std::tuple<int, int, int, int>(g, item.right - s + 1, m_lItems[item.right - s +
                                                                                                  1][s].r2l_solid_both[GRAND_INDEX(
                                g, s, item.right - s + 1)].bestItem().getSplit(), s));
